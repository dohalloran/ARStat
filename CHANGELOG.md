...
- Made uploaded-file state identity content-aware (including schema), so replacing a file with corrected data under the same filename and dimensions reruns assay/layout auto-detection instead of reusing stale state.
...
- Replicate, well, and experiment-ID columns are never offered as XY response columns. XY defaults are chosen only from `RepN`/`YN`-style headers.
- Blocked XY tables in which a dose repeats within the same group/drug, which previously pooled several curves or long-form rows without warning.
- Wide `RepN` tables now open in the normalized XY layout automatically.
- Duplicate column headers are now detected. pandas silently renames a second `L1` to `L1.1`, so the earlier exact-name check never triggered.
- Count columns that are really proportions (every row totals ≤ 1) now block the run instead of only producing a warning.
