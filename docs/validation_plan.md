...
2. Count-response calculations for egg hatch and larval development assays.
...
The script `scripts/generate_simulated_benchmarks.py` creates synthetic datasets for all three assay workflows with known IC50 values. Count endpoints are generated using binomial sampling. Motility endpoints are generated as continuous activity measurements that decline with the known inhibition curve and are then normalized through the same raw-motility function used by the app.
